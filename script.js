...
function preloadImages() {
    for (const [modalImage, gameImage] of Object.entries(avatarImageMappings)) {
        ...
        modalImg.onerror = () => console.error(`Failed to load image: ${modalImage}`);
        avatarImages[modalImage] = modalImg;

        const gameImg = new Image();
        gameImg.src = gameImage;
        gameImg.onerror = () => console.error(`Failed to load image: ${gameImage}`);
...
function drawCollectedSBTs() {
    ...
    for (const [sbtName, count] of Object.entries(totalCollectedSBTs)) {
        if (!collectibleImgs[sbtName]) continue;
    ...
    collectionEffects.forEach((effect, index) => {
        ...
        if (img && img.complete) {
            ...
            if (progress <= 1) {
                ...
                ctx.drawImage(img, -effect.size * scale / 2, -effect.size * scale / 2, effect.size * scale, effect.size * scale);
                ctx.restore();
            }
        }
    });

    collectionEffects = collectionEffects.filter(effect => (frame - effect.startFrame) <= effect.duration);
}
